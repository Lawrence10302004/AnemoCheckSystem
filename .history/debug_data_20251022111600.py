...
try:
    ...
    print(f"Error accessing imported_files table: {e}")
    # Check if table exists
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='imported_files'")
    table_exists = cursor.fetchone()
    print(f"imported_files table exists: {table_exists is not None}")

# Check if file_id column exists in classification_import_data
print("\n=== TABLE STRUCTURE ===")
cursor.execute("PRAGMA table_info(classification_import_data)")
columns = cursor.fetchall()
print("classification_import_data columns:")
for col in columns:
    print(f"  {col[1]} ({col[2]})")
    
# Check if any records have file_id
cursor.execute('SELECT file_id, COUNT(*) FROM classification_import_data GROUP BY file_id')
file_ids = cursor.fetchall()
print(f"File IDs in classification_import_data: {file_ids}")

...
def add_imported_classification_data(age, gender, category):
    """Add imported classification data to the database."""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    try:
        # Insert into a new table for imported data
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS imported_classification_data (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # Insert the data
        cursor.execute('''
        INSERT INTO imported_classification_data (age, gender, category)
        VALUES (?, ?, ?)
        ''', (age, gender, category))
...
def get_combined_charts_data():
    """Get combined chart data from both user data and imported data."""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Age groups distribution (from users + imported data)
    cursor.execute("""
        SELECT 
            CASE 
        ...
        FROM (
            ...
            SELECT age
            FROM imported_classification_data
        )
        GROUP BY age_group
        ORDER BY 
            CASE age_group
    ...
    # Gender distribution (from users + imported data)
    cursor.execute("""
        SELECT gender, COUNT(*) as count
        FROM (
            SELECT gender
    ...
    # Severity classification distribution (from classification history + imported data)
    cursor.execute("""
        SELECT 
            CASE 
                WHEN predicted_class LIKE '%Normal%' OR predicted_class LIKE '%normal%' THEN 'Normal'
        ...
        FROM (
            ...
            SELECT category as predicted_class
            FROM imported_classification_data
        )
        GROUP BY severity
        ORDER BY 
